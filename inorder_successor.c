...
void pre_suc_inorder(struct node *root,struct node *pre,struct node *suc,int key);
main( )
{
	struct node *root=NULL,*ptr,*suc,*pre;
	suc=pre=NULL;
	int choice,k;
	...
	while(1)
	{
		...
		case 3:
		 ...
		 printf("Enter the key to be searched: ");
			scanf("%d",&k);
			 pre_suc_inorder(root,pre,suc, k);
			printf(" Inorder succersor is %d\n",suc->info);
...
void  pre_suc_inorder(struct node *root,struct node *pre,struct node *suc,int key)
{
 ...
 {
     suc =root;
      pre_suc_inorder(root->lchild,pre,suc,key);
 }
    else
        {
        pre=root;
         pre_suc_inorder(root->rchild,pre,suc,key);
    }
